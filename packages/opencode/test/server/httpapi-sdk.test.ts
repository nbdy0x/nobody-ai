...
const original = {
  NOBODY_SERVER_PASSWORD: Flag.NOBODY_SERVER_PASSWORD,
  NOBODY_SERVER_USERNAME: Flag.NOBODY_SERVER_USERNAME,
}
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.NOBODY_SERVER_PASSWORD = input?.password
      Flag.NOBODY_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".nobody", "skills", "project-rest-skill", "SKILL.md"),
      `---
...
afterEach(async () => {
  Flag.NOBODY_SERVER_PASSWORD = original.NOBODY_SERVER_PASSWORD
  Flag.NOBODY_SERVER_USERNAME = original.NOBODY_SERVER_USERNAME
  await disposeAllInstances()
  await resetDatabase()
})
