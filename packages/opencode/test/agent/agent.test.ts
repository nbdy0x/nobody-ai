...
it.instance("plan agent denies edits except .nobody/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".nobody/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".nobody", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NOBODY_TEST_HOME
      process.env.NOBODY_TEST_HOME = test.directory
