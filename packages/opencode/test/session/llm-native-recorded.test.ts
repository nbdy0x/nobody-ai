import { ConfigV1 } from "@nobody0x/core/v1/config/config"
import { SessionV1 } from "@nobody0x/core/v1/session"
import { ModelsDev } from "@nobody0x/core/models-dev"
import { HttpRecorder } from "@nobody0x/http-recorder"
import { HttpRecorderInternal } from "@nobody0x/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@nobody0x/llm"
import { RequestExecutor } from "@nobody0x/llm/route"
...
const zenURL = (connection: string) => `https://console.nobody0x.com/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NOBODY_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("NOBODY_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NOBODY_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  {
    id: "opencode-proxy",
    name: "Nobody AI proxy",
    providerID: ProviderV2.ID.nobody,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NOBODY_RECORD_CONSOLE_TOKEN && process.env.NOBODY_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.nobody,
        name: "Nobody AI Zen",
        env: ["NOBODY_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NOBODY_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NOBODY_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NOBODY_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NOBODY_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NOBODY_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NOBODY_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NOBODY_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "nobody.json"),
      JSON.stringify({ $schema: "https://nobody0x.com/config.json", ...scenario.config(model) }),
    ),
  )
