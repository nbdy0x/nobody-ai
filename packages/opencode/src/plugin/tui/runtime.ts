...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@nobody0x/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@nobody0x/core/effect/app-node-builder"
import { errorData, errorMessage } from "@nobody0x/tui/util/error"
...
import { hasTheme, upsertTheme } from "@nobody0x/tui/context/theme"
import { Global } from "@nobody0x/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@nobody0x/core/util/flock"
import { Flag } from "@nobody0x/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@nobody0x/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@nobody0x/core/v1/config/plugin"
import { createCommandShim } from "@nobody0x/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@nobody0x/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".nobody"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".nobody", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".nobody", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NOBODY_PURE ? [] : pluginOrigins
    if (Flag.NOBODY_PURE && pluginOrigins.length) {
    }
