...
import { isRecord } from "@nobody0x/tui/util/record"
...
import { TuiKeybind } from "@nobody0x/tui/config/keybind"
...
import { TuiConfig } from "@nobody0x/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old nobody.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.nobody`
  // folders between cwd and home, and NOBODY_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NOBODY_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NOBODY_TUI_CONFIG override, if set.
  if (Flag.NOBODY_TUI_CONFIG) {
    const configFile = Flag.NOBODY_TUI_CONFIG
  ...
  // 4. `.nobody` directories (and NOBODY_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".nobody") || dir === Flag.NOBODY_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".nobody") && dir !== Flag.NOBODY_CONFIG_DIR) continue
