...
import { Flag } from "@nobody0x/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { InstallationLocal, InstallationVersion } from "@nobody0x/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@nobody0x/core/v1/config/console-state"
import { FSUtil } from "@nobody0x/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@nobody0x/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@nobody0x/core/v1/config/config"
import { RemoteAuthError } from "@nobody0x/core/v1/config/error"
import { ConfigPermissionV1 } from "@nobody0x/core/v1/config/permission"
import { ConfigPluginV1 } from "@nobody0x/core/v1/config/plugin"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://nobody0x.com/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://nobody0x.com/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://nobody0x.com/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://nobody0x.com/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://nobody0x.com/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("nobody-ai", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
