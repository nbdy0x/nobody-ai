...
import type { Hooks } from "@nobody0x/plugin"
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (provider === "opencode") {
      yield* Prompt.log.info("Create an api key at https://nobody0x.com/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://nobody0x.com/docs/providers/#cloudflare-ai-gateway",
      )
    }
